...
from xelib.helpers import get_string, validate
from xelib.elements import element_context


def element_to_json(id_):
    return get_string(lambda len_: raw_api.ElementToJson(id_, len_),
...
def element_from_json(id_, path, json):
    validate(raw_api.ElementFromJson(id_, path, json),
             f'Failed to deserialize element from JSON: '
